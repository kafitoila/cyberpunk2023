...
async def get_sender(message: Message, command: CommandObject, state: FSMContext):
    if not command.args:
        await message.answer(f'Для создания кампании для рассылки введи команду /sender и имя рассылки')
        return
...
async def confirm(message: Message, bot: Bot, message_id: int, chat_id: int, reply_markup: InlineKeyboardMarkup = None):
    ...
    await message.answer(f'Вот сообщение, которое будет отправлено. Подтверди.',
