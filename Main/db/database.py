...
class Request:
    ...
    async def check_tg_id_assigned(self, tg_id):
        query = f"SELECT EXISTS (SELECT 1 FROM users WHERE tg_id = {tg_id} AND is_dead = False);"
        return await self.connector.fetchval(query)
    ...
    async def get_user_id(self, tg_id):
        query = f"SELECT user_id FROM users WHERE tg_id = {tg_id} AND is_dead = False;"
        return await self.connector.fetchval(query)
    ...
    async def get_org_name_by_tg_id(self, tg_id):
        ...
        f"where u.tg_id = {tg_id} AND u.is_dead = False;"
        return await self.connector.fetchval(query)
